...
Console::LogStream Console::log(Level lvl, const char* tag) {
    bool active = (static_cast<int>(lvl) >= static_cast<int>(getMinLevel())); // 级别过滤
    if (active && tag && !getTagEnabled(tag)) {                              // 标签过滤
        active = false;
    }
...
Console::Level Console::getMinLevel() {
    std::lock_guard<std::mutex> lock(s_filterMutex);
    return s_minLevel;
}
...
bool Console::getTagEnabled(const std::string& tag) {
    std::lock_guard<std::mutex> lock(s_filterMutex);
